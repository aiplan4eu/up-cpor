...
namespace CPORLib.Algorithms
{
    ...
    public class SDRPlanner
    {
        ...
        public static Options.DeadendStrategies DeadendStrategy = Options.DeadendStrategies.Lazy;


        //TODO - to remove
        public int amount_of_offline_pruned_states;

        public SDRPlanner(string sPath, string DeadEndFile, Domain d, Problem p, bool bOnline, Options.DeadendStrategies ds)
        {
        ...
        {
            ComputeCompletePlanTree = !bOnline;
            DeadendStrategy = Options.DeadendStrategies.Lazy;
        ...
        private List<string> RunPlannerNoFiles(MemoryStream msModels, int iIndex)
        {
            ...
            if (Planner == Planners.FF)
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\ff.exe";
            if (Planner == Planners.FF_WSL)
            {
                p.StartInfo.WorkingDirectory = Options.BASE_PATH + @"\Planners\";
        ...
        private bool RunPlannerWithFiles(string sPath, int iIndex)
        {
            ...
            if (Planner == Planners.FFX)
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\ff-x.exe";
            if (Planner == Planners.MIPS)
            {
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\mips-xxl.exe";
                p.StartInfo.Arguments = "-O ";
            }
            if (Planner == Planners.FFsa)
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\ffsa.exe";
            if (Planner == Planners.FFha)
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\ffha.exe";
            if (Planner == Planners.MetricFF)
            {
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\metric-ff.exe";
                p.StartInfo.Arguments = "-O ";
            }
            if (Planner == Planners.LPG)
            {
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\lpg-td-1.0.exe";
                p.StartInfo.Arguments = "-n 1 ";
            }
            if (Planner == Planners.CPT)
            {
                p.StartInfo.FileName = Options.BASE_PATH + @"\Planners\cpt-1.0.exe";
                Options.RedirectShellOutput = true;
            }
            ...
            p.StartInfo.UseShellExecute = false;
            if (Options.RedirectShellOutput || Planner == Planners.FFX)
            {
                m_sFFOutput = "";
                p.StartInfo.RedirectStandardOutput = true;
                p.OutputDataReceived += new DataReceivedEventHandler(OutputHandler);
        ...
        public bool RunProcess(Process p, string sInputFile)
        {
            p.StartInfo.UseShellExecute = false;
            FFProcesses[Thread.CurrentThread] = p;
            if (Options.RedirectShellOutput)
            {
                p.StartInfo.RedirectStandardOutput = true;
                p.OutputDataReceived += new DataReceivedEventHandler(OutputHandler);
            }
