...
namespace CPORLib.LogicalUtilities
{
    public class ParametrizedPredicate : Predicate
    {
        ...
        protected override string GetString()
        {
            ...
            if (Negation)
                s = "(not ";
            s += "(" + Name + " " + Utilities.ListToString(m_lParameters) + ")";
            if (Negation)
                s += ")";
            return s;
        }
            ...
            if (bAllGrounded)
            {
                if (gpred.Name == "=")
                {
                    ...
                    if (bSame && !Negation || !bSame && Negation)
                        return Utilities.TRUE_PREDICATE;
                    else
                        return Utilities.FALSE_PREDICATE;
                }
                return gpred;
            }
            else
        ...
        public override Predicate GenerateGiven(string sTag)
        {
            ...
            pGiven.AddParameter(new Parameter(Utilities.TAG, sTag));
            if (Negation)
