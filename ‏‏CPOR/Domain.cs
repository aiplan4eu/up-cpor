...
*/
        ...
        private Action GetActionByName(string sActionName)
        {
            foreach (Action a in Actions)
            {
                ...
                if (a.Name.ToLower().Replace(DELIMITER_CHAR, "-") == sActionName)
                    return a;
            }
            ...
            foreach (Action a in Actions)
            {
                ...
                if (sActionName.StartsWith(a.Name.ToLower().Replace(DELIMITER_CHAR, "-"))) //assuming that this is a splitted conditional effect action. BUGBUG - need better solution for this
                {
                    if (aBestMatch == null || aBestMatch.Name.Length < a.Name.Length)
                        aBestMatch = a;
                }
        ...
        public Action GroundActionByName(string[] asAction, IEnumerable<Predicate> lPredicates, bool bContainsNegations)
        {
            ...
            if (pa.Preconditions == null || fGroundedPreconditions.ContainedIn(lPredicates, bContainsNegations))
            {
                ...
                foreach (Parameter p in pa.Parameters)
                    sName += DELIMITER_CHAR + dBindings[p].Name;
        ...
        public List<Action> GroundAllActions(Problem problem, bool bRemoveConstantPredicates)
        {
            ...
            {
                foreach (Action a in lAllGrounded)
                {
        ...
        private void WriteSimpleChooseAction(StreamWriter sw, GroundedPredicate p, int idx, int cInits, bool bValue)
        {
            ...
            foreach (Constant c in p.Constants)
                spName += DELIMITER_CHAR + c.Name;
            spName += DELIMITER_CHAR + idx;
            if (bValue)
                spName += "_T";
            else
                spName += "_F";
